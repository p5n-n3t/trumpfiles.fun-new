...
function validateEmailPayload(payload: EmailPayload): ValidatedEmail | null {
  ...
  const text = typeof payload.message === "string" ? payload.message.replace(/\u0000/g, "").trim() : "";
...
function buildEmailText(email: ValidatedEmail): string {
